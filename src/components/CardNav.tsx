...
const CardNav: React.FC<CardNavProps> = ({
  ...
  baseColor = '#fff', menuColor, buttonBgColor, buttonTextColor,
  buttonLabel = 'Get Started', onButtonClick
}) => {
              ...
              <div className="nav-card-links mt-auto flex flex-col gap-[2px]">
                {item.links?.map((lnk, i) => (
                  <a
                    ...
                    onClick={(e) => { if (lnk.onClick) { e.preventDefault(); lnk.onClick(); } }}
                  >
                    <ArrowUpRight weight="bold" size={16} className="nav-card-link-icon shrink-0" aria-hidden="true" />
                    {lnk.label}
                  </a>
                ))}
              </div>
